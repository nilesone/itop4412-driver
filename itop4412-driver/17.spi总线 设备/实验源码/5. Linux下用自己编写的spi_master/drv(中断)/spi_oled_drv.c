...
{
	//unsigned char buf[15] = {0xe2, 0xa2, 0xa0, 0xc8, 0xa4, 0xa6, 0x25, 0x81, 0x1a, 0x2f, 0x40, 0xaf};
	
    st7567_reset(); 		 //复位 LCD
    mdelay(100);
    /* lcd初始化，看官方提供的例程 */
	//spi_write(oled_spi_device, &buf, 12);
...
static int spi_oled_probe(struct spi_device *spi)
{
	...
	unsigned char txbuf[2] = {0xfe, 0xff};
	...
	printk("spi_oled_probe spi_dev_addr = 0x%x, spi_master_addr = 0x%x\n", spi, spi->master);
	...
	printk("rxbuf[0] = 0x%x, rcbuf[1] = 0x%x\n", rxbuf[0], rxbuf[1]);
